...
def create_filter_panel(crime_df, network_df):
    """
    Creates the slide-down filter panel.
    """
    ...
    panel = html.Div(
        ...
        className="filter-slide-panel filter-hidden", # Start hidden
        children=[
            # A container for all the filter inputs
            html.Div(
                className="filter-inputs-wrapper",
                children=[
                    html.Div(className="filter-control", children=[html.Label("Time Range (Crimes)"), time_slider]),
                    html.Div(className="filter-control", children=[html.Label("NAIN Range (Network)"), nain_slider]),
                    html.Div(className="filter-control", children=[html.Label("Crime Types"), crime_type_dropdown]),
                ]
            ),
            # The apply button is outside the wrapper for separate styling
