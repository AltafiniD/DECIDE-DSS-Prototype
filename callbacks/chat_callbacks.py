...
LAYER_INFO = {
    ...
    "buildings": {
        "text": "This layer shows an integrated dataset of building footprints. You can colour them by different flood risk types using the filter panel at the bottom.",
        "questions": ["Where does this data come from?", "Which are the buildings under hazard? How can I visualise them?"]
    },
    "flooding_toggle": {
        "text": "This layer toggles the flood hazard zones. Potential hazards from the sea, rivers, surface water and other watercourses can be visualised here. You can select which flood maps to combine and display in the filter panel.",
        "questions": ["Using the filters.", "What do the different colours mean?", "How to visualise buildings under flood hazard?", "How to visualise flood risk in road-networks?", "What are the recommendations?", "Where does this data come from?"]
    },
    "network": {
        ...
        "questions": ["Using the filters.", "What does configurational analysis mean?", "What is Connectivity?", "What Integration (NAIN) shows?", "What is Route Choice (NACH)?", "What Kemeny-based Centrality (KBC) shows?", "What is Degree Centrality (NADC)?", "What can I see through the histograms?","How can I visualise the roads at risk of flooding?"]
    },
    "crime_master_toggle": {
        "text": "This layer toggles the crime data layers, related to antisocial behaviour in the city. You can view crimes as individual points or as a heatmap showing density. Use the sub-options to switch between views.",
    ...
    "deprivation": {
        ...
        "questions": ["What is WIMD?", "What do the colours represent?"]
    },
    "land_use": {
        "text": "This layer displays the different types of land use across the city, such as residential, commercial, and green spaces, based on the UK's National Land Use database.",
        "questions": ["How can I filter by land use?", "Why is this important?"]
    },
    "population": {
        "text": "This layer visualizes population density data from the 2021 census for different Output Areas (OAs), the smallest geographical unit for census data.",
...
PREDEFINED_ANSWERS = {
    ...
    "Which are the buildings under hazard? How can I visualise them?": "By choosing a building risk type in the filter panel (Buildings at Risk), you can visualize which buildings are at risk from specific flood hazards. They will appear alongside the flood areas.",
    # Flooding
    "Using the filters.": "Open the filter panel at the bottom of the screen. In Building & Environmental, you can select the flooding hazard layers and combine different hazard sources. You can also see the buildings under hazard by selecting the drop-down menu. In the network tab, you can select the flood risk indexes, combining flood hazards with road-network performance metrics",
    "How to visualise buildings under flood hazard?": "You can choose a flood hazard type (rivers, sea or surface water) in the filter panel (buildings under hazard), this will highlight which buildings are under each specific flood hazard and their location.",
    "How to visualise flood risk in road-networks?": "You can choose a risk index in the filter panel (Network Analysis) that will combine a system performance metric, indicating vulnerability, and a flood hazard. You can then see how performance would be affected by a flooding event, and which road interruptions would cause more impacts through the metrics of accessibility (NAIN), preferential routes (NACH) or redundancy (KBC).",
    "What do the different colours mean?": "Different colours represent different flood hazard types, their recurrence and intensity. Lighter colours indicate a lower recurrence interval, but a higher intensity in flooding (more area flooded), while darker colours indicate a higher recurrence interval, but a lower intensity in flooding (less area flooded).",
    "What are the recommendations?": "Areas identified as under hazard or risk may require structural interventions to protect the buildings and roads: i.e. improved drainage systems, or flood defences. You can start interpreting the data by identifying the roads with the least redundancy and highest impacts towards accessibility through using the histograms. Furthermore, you can see the areas that are under hazard and structure emergency response. Focus should be given to surface water, as this is a recurrent disruption to urban movement.",
    ...
    "What can I see through the histograms?": "The histograms showcase the distribution of values for the selected network measure, and let you examine the streets based on how important they are for accessibility, used as preferential routes or redundant for the system. Histograms are divided in 10 quartiles with an equal count of streets to each that show the road-network performance from low to high. Clicking on a quartile will filter the streets that fall within that range, and showcase their hierarchical importance within this selection. This can be used to identify points of intervention or analyse the network structure.",
    "Using the filters.": "You can use the filters in the bottom panel. In the section 'Network Analysis' you can select which measure to display. You can also select a range of values, that will be displayed on the histogram - the default visualization shows the whole network.",
    "What Integration (NAIN) shows?": "Integration (or Normalised Angular Integration - NAIN) is a global measure that informs how easily a street can be reached from all other streets in the network - or its relative accessibility. Streets with high integration values are more accessible and are often located in central areas, while streets with low integration values are less accessible and typically found in peripheral areas. This information is useful for urban analytics, as it identifies the roads that tend to attract more movement and activity.",
    ...
    "What Kemeny-based Centrality (KBC) shows?": "Kementrality (or Kemeny-based Centrality - KBC) measures the overall redundancies of a system, identifying the roads that establish bridges among the diverse spaces. Streets with low KBC present more alternatives, having more redundancies, while streets with high KBC have less redundancy, meaning they are more critical for maintaining the connection structure of the network. If removed, they may lead to significant changes in the system's accessibility. This information is useful for urban analytics as it helps to identify streets that are crucial for the network structure, as once removed, they may lead to a partial collapse in accessibility.",
    ...
    "What is Connectivity?": "Connectivity counts the number of direct connections a street has to other streets in the network, Angular Connectivity multiplies connectivity by the angle of the street. Streets that have a greater angle in between them have more impedance to movement when compared to streets with a lower angle, as individuals tend to prefer moving through areas that are more visible, so straighter. ",
    "How can I visualise the roads at risk of flooding?": "In the 'Network Analysis' tab, you can select risk metrics that combine each flood hazard with each configurational metric. These showcase the roads at risk, informing the roads that would be most affected in terms of performance when impacted by flooding.",
    # Crime
    ...
    "What do the colours represent?": "The map colours areas based on their deprivation percentile. Darker blue areas are more deprived compared to lighter areas, which are less deprived.",
    # Land Use
